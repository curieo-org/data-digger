...
async def ingest_tbl_studies_info(
    ...
    await transfer_table_data(
        source_pg_engine, target_pg_engine, table_structure, database_reader
    )
...
async def ingest_tbl_baseline_details(
    ...
    table_structure = TableStructure(
        ...
        columns=[
            ...
            "baseline_title",
            "title",
            "baseline_measurement_details",
            "baseline_group_details",
        ],
...
async def ingest_tbl_primary_outcome_measurement(
    ...
    table_structure = TableStructure(
        ...
        columns=[
            "nct_id",
            "title",
            "outcome_primary_measurement_details",
            "outcome_primary_measurement_value_details",
        ],
        primary_keys=["nct_id"],
    )
...
async def ingest_tbl_secondary_outcome_measurement(
    ...
    table_structure = TableStructure(
        ...
        columns=[
            ...
            "outcome_secondary_measurement_value_details",
        ],
        primary_keys=["nct_id"],
    )
...
async def ingest_all_tables(settings: Settings) -> None:
    source_pg_engine = PGEngine(
        settings.psql.temporary_db.get_secret_value(), settings.database_reader
    )

    target_pg_engine = PGEngine(
        settings.psql.main_db.get_secret_value(), settings.database_reader
    )
    ...
    await ingest_tbl_studies_info(
        source_pg_engine,
        target_pg_engine,
        vector_transfer_processor,
        settings.database_reader,
    )
    await ingest_tbl_baseline_details(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_primary_outcome_measurement(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_secondary_outcome_measurement(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_adverse_details(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_arms_details(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_conditions(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_design_outcomes(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_designs(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_eligibilities(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_interventions(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_pubmed_links(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
    await ingest_tbl_studies_sponsors(
        source_pg_engine, target_pg_engine, settings.database_reader
    )
