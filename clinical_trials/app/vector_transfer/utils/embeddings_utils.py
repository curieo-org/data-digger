...
class EmbeddingUtil:
    ...
    async def calculate_dense_embeddings(self, nodes: List[CurieoBaseNode]):
        ...
        id_to_dense_embedding = {
            node.id_: dense_embedding
            for node, dense_embedding in zip(nodes, dense_embeddings)
        }
    ...
    async def calculate_sparse_embeddings(self, nodes: List[CurieoBaseNode]):
        ...
        indices = [
            [entry.get("index") for entry in sublist] for sublist in sparse_embeddings
        ]
        vectors = [
            [entry.get("value") for entry in sublist] for sublist in sparse_embeddings
        ]

        assert len(indices) == len(vectors) == len(nodes)

        id_to_sparse_embedding = {
            node.id_: {"indices": index, "vector": vector}
            for node, index, vector in zip(nodes, indices, vectors)
        }

        return id_to_sparse_embedding

    async def calculate_dense_sparse_embeddings(
        self, nodes: List[CurieoBaseNode]
    ) -> List[CurieoBaseNode]:
        ...
        for node in nodes:
            node.embedding, node.sparse_embedding = (
                id_to_dense_embedding[node.id_],
                id_to_sparse_embedding[node.id_],
            )
