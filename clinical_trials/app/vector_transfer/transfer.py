...
from app.vector_transfer.database_reader import DatabaseReader, VectorIngestionStatus
...
class VectorTransferProcessor:
    def __init__(self, database_engine: PGEngine, settings: Settings):
        """
    ...
    async def node_metadata_transform(
        ...
        columns_to_be_added = list(metadata.keys())
    ...
    async def process_single_row(
        self, table_structure: TableStructure, row: dict
    ) -> None:
        try:
            ...
            self.database_reader.update_ingestion_status(
                primary_key_values, VectorIngestionStatus.IN_PROGRESS
            )

            embeddable_data = row.get("embeddable_data")
            metadata = row.get("metadata")

            text_value = " ".join([str(value) for value in embeddable_data.values()])

            nodes = [
                CurieoBaseNode.from_text_node(text_node)
                for text_node in self.create_nodes(text_value)
            ]
            nodes_with_embeddings = await self.eu.calculate_dense_sparse_embeddings(
                nodes
            )

            nodes_ready_to_be_added = await self.node_metadata_transform(
                nodes_with_embeddings, metadata
            )
            ...
            self.database_reader.update_ingestion_status(
                primary_key_values, VectorIngestionStatus.COMPLETED
            )
        except Exception as e:
            logger.exception(f"Error in processing row: {e}")
            self.database_reader.update_ingestion_status(
                primary_key_values, VectorIngestionStatus.FAILED
            )
    ...
    def insert_nodes_into_index(
        self, node_identifiers: dict, nodes_to_be_added: List[CurieoBaseNode]
    ) -> None:
        try:
            conditions = [
    ...
    async def database_to_vectors(self, table_structure: TableStructure) -> None:
        ...
        for rows in tqdm(
                ...
                for row in rows:
                    tasks.append(
                        event_loop.run_in_executor(
                            executor, self.process_single_row_sync, table_structure, row
                        )
                    )
            ...
            logger.info(
                f"Processed Batch size of {self.batch_size} in {time.time() - start_time:.2f}s"
            )
