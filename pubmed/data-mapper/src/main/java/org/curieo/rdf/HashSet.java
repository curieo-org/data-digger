...
public class HashSet<E> extends AbstractSet<E> implements Serializable {
	 ...
	 * started with a size of 2, we'd have to expand as soon as the second item was
	 * added.
	 */
	...
	/**
	 * Number of objects in this set; transient due to custom serialization. Default
	...
	/**
	 * Removes the item at the specified index, and performs internal management to
	 * make sure we don't wind up with a hole in the table. Default access to avoid
	 ...
	 * index of an empty slot in the table where this item should be inserted if it
	...
	/**
	 * Tricky, we left a hole in the map, which we have to fill. The only way to do
	 * this is to search forwards through the map shuffling back values that match
	 * this index until we hit a null.
	 */
