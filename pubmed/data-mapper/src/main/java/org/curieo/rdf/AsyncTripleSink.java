...
/**
 * An asynchronous triple store puts the triple store in another thread, allowing you to generate
 * the triples in another (main) thread. When done, call .get() to complete the storage of triples.
...
public class AsyncTripleSink<T extends TripleStore> implements TripleStore, Supplier<T> {
  ...
  /**
   * Implementation according to interface, but always returning null You cannot use the triple
   * right away as the storage is asynchronous
   *
          ...
          if (!message.done) {
            if (message.literal == null) {
              if (message.triple != null) {
                ...
                store.assertAndNameTriple(
                    message.uri, message.subject, message.verb, message.object);
              } else {
