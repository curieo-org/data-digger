...
/**
 * Store holding all Triples in Memory
 * And yes, that is not a sustainable solution for any kind of real RDF data set.
 */
public class Store implements Collection<RdfTriple>, TripleStore, TripleIterable {
	public static final String INDENT = "  ";
	
	/**
	 * Indexed to the URI - that must always be unique.
	 * You *can* insert triples without URI -- those are anonymous 
	 * are silently de-duplicated if inserted multiple times.
	 */
	...
	/**
	 * Assert a triple.
	 * If the triple is *anonymous* - that is, it has no name (URI) we will *not* allow
	 * the triple to be duplicated. In this case the method returns the existing triple.
	...
	public RdfTriple accept(RdfTriple triple) {
		...
		if (uri == null) {
			if (triple.isSimple()) {
				Optional<RdfTriple> first = getTriples(triple.getSubject(), triple.getVerb(), triple.getObject()).findFirst();
				if (first.isPresent()) {
					return first.get();
				}
			}
				...
				for (RdfTriple c : getBySubjectAndVerb(triple.getSubject(), triple.getVerb())) {
					if (c.isSimple()) {
						LOGGER.info("Triple [{} {}] once asserted with literal ({}) and once with URI object ({})",
								triple.getSubject(),
								triple.getVerb(),
	...
	/**
	 * Get all object values - if one is a container, return these
	 * values as a list
	 *
	 * @param s
	 * @param v
	...
	public List<String> getContainer(String s, String v) {
		...
		for (String object : new RdfTripleIterable(safeIntersection(bySubject.get(s), byVerb.get(v))).objects()) {
			int size = values.size();
			getBySubject(object).filter(Store::isMember).forEach(t -> values.add(new String[] { t.getVerb(), t.getObject() }));
			if (values.size() == size) {
				values.add(new String[] { null, object });
			}
		}
		return values.stream().sorted((a, b) -> StringUtils.compare(a[0], b[0])).map(a -> a[1]).collect(Collectors.toList());
	}
	
	private static boolean isMember(RdfTriple triple) {
		return triple.getVerb().startsWith(MEMBER) && StringUtils.isNumeric(triple.getVerb().substring(MEMBER.length()));
	}
	...
	public TripleIterable getByVerb(String... verbs) {
		if (verbs.length == 0) {
			...
			return Stream.of(verbs).flatMap(v -> new RdfTripleIterable(byVerb.getOrDefault(v, emptySet())).stream())::iterator;
		}
	}
	...
	/**
	 * Does the *transitive* relation src-REL-trg exist in this graph?
	 * If not, does the relation src-REL-?-REL-trg hold?
	 * If src=trg this method returns true
	 * 
	 * @param source
	 * @param relation
	 * @param target
	...
	/**
	 * Remove an evo:category when two concepts are the same and in the same category
	 * We find the all the categories from the originalConcept and the conceptWithDuplicates
	 * abd remove the ones that have in common from the conceptWithDuplicates
	 *
	 * @param conceptWithDuplicates - The string Uri that we will check its categories and remove the common ones
	 ...
	 * @return number of replacements - The number of categories that were removed from conceptWithDuplicates
	 */
	public int removeDuplicateCategoriesFromConcept(final String conceptWithDuplicates, final String originalConcept, Set<String> preservedCategories) {
		int repl = 0;
		...
		this.getBySubjectAndVerb(originalConcept, Constants.EVO_CATEGORY).objects().forEach(categoryOriginalConcept::add);
		this.getBySubjectAndVerb(conceptWithDuplicates, Constants.EVO_CATEGORY).forEach(categoryConceptWithDuplicates::add);

		for (RdfTriple categoryTriple : categoryConceptWithDuplicates) {
			String candidateCategory = categoryTriple.getObject();
			if (categoryOriginalConcept.contains(candidateCategory) && !preservedCategories.contains(candidateCategory)) {
	 ...
	 * @return number of replacements - The number of categories that were removed from conceptWithDuplicates
	 */
	public int removeDuplicateCategoriesFromConcept(final String conceptWithDuplicates, final String originalConcept) {
	...
	public int replaceUri(final String olduri, final String newuri) {
		...
		if (set != null) {
			...
			for (RdfTriple rt : removed) {
				if (rt.isSimple())
					assertTriple(rt.getUri(), newuri, rt.getVerb(), rt.getObject());
				else
					assertTriple(rt.getUri(), newuri, rt.getVerb(), rt.getLiteralObject());
			}
			repl += removed.size();
		}
		...
		if (set != null) {
			...
			for (RdfTriple rt : removed) {
				if (rt.isSimple())
					assertTriple(rt.getUri(), rt.getSubject(), newuri, rt.getObject());
				else
					assertTriple(rt.getUri(), rt.getSubject(), newuri, rt.getLiteralObject());
			}
			repl += removed.size();
		}
	...
	private static Iterable<Integer> safeIntersection(Set<Integer> a, Set<Integer> b, Set<Integer> c) {
	...
	public boolean contains(String subject, String verb, String object) {
		return safeIntersection(bySubject.get(subject), byVerb.get(verb), byObject.get(object)).iterator().hasNext();
	}
	...
	public boolean add(RdfTriple e) {
		...
		// return true if changed... in the situation where there was a triple that blocks this one from being add
	...
	private final Map<String, Map<Literal, RdfVerbLiteral>> immutableVerbLiteralPairs = new HashMap<>();
	private final Set<String> verbsImplyingImmutability = new HashSet<>();
	
	/**
	 * As a memory and speed optimization, you can define specific RDF verbs
	 * that causes triples with this verb to be stored as read-only.
	 * 
	 * @param verb
	 * @return this
	 */
	...
	/**
	 * Add a verb-object pair that is always stored together, and as an immutable pair.
	 * This saves both on memory and speed.
	 * @param v
	 * @param o
	...
	private RdfVerbObject addImmutableVerbObjectPair(String v, String o) {
		...
		immutableVerbObjectPairs
			.computeIfAbsent(v, verb -> new HashMap<>())
			.put(o, rvo);
		return rvo;
	}
	...
	/**
	 * For a given relationship (transitive, e.g. broader/narrower skos relation) find the set of 
	 * URIs that relate to this subject n this relationship.
	 * The original subject URI is also included!
	 * 
	 * @param subject
	 * @param verbs
	...
	public boolean hasAnyProperty(String uri, Collection<String> properties) {
		return properties.stream().anyMatch(property -> this.getBySubjectAndVerb(uri, property).iterator().hasNext());
	}
