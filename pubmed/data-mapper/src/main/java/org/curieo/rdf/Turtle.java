...
/**
 * This class is a reader for Turtle and Trig files.
 * These files are read using the same method {@link read} which returns a 
 * {@code Map<>} from graph identifier to graphs.
 * If there are no named graphs, the triples read are under the
 * {@code DEFAULT} graph, so for turtle (non-Trig) files, the 
 * read operation could read {@code turtle.read(file).get(DEFAULT);}.
 * The Turtle reader needs an {@code Store} supplier - if 
 * you have default namespace services or abbreviation optimizations
...
public class Turtle {
	...
	private Map<String, Store> parseTurtle(ParseParameters pf) {
		...
		return graphs.entrySet().stream().collect(Collectors.toMap(Entry::getKey, e -> e.getValue().get()));
	}
				...
				if (token.equals("@prefix")) {
					...
					if (currentGraph.equals(DEFAULT)) {
						...
						if (pf.peek(0) == '{') {
							...
							graphs.computeIfAbsent(currentGraph, g -> {
								Store t = storeSupplier.get();
								ns.knownPrefixes().forEach(p -> t.getNamespaceService().put(p, ns.getFullForm(p)));
					...
					if (reification != null) {
						...
						graphs.get(currentGraph).assertAndNameTriple(id, reification[0], reification[1], reification[2]);
	...
	private static void assertTriple(TripleStore rdf, RdfItem sr, RdfItem vr, RdfItem o) {
		...
		if (s == null) throw new NullPointerException("Subject cannot be null (v = " + v + "), (o = " + o  + ")");
		if (v == null) throw new NullPointerException("Verb cannot be null (s = " + s + "), (o = " + o  + ")");
		if (o == null) throw new NullPointerException("Object cannot be null (s = " + s + "), (v = " + v  + ")");
		if (o.isLiteral()) {
			rdf.assertTriple(s, v, o.getLiteral());
		} else {
			if (o.getString() == null) throw new NullPointerException("Object value cannot be null (s = " + s + "), (v = " + v  + ")");
		...
		if (pf.next() == '"') {
			...
			switch (pf.next()) {
				case '"':
					...
					return new RdfItem(new Literal(parseTripleQuotedString(pf, '"'), optionalLanguage(pf), null));
				case '^':
					pf.eat();
					if (pf.eat() != '^') {
	...
	private static final Pattern REIFICATION_PATTERN = Pattern.compile("urn:triple:([^:]+):([^:]+):([^:]+)");
