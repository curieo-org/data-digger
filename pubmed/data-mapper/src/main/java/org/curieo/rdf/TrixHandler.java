...
/**
 * Trix Handler
 * -- reading Rdf Xml, returning a Store
 * -- a taxonomy-to-triple store loader
 *
 * cf. https://docs.oracle.com/javase/tutorial/jaxp/sax/parsing.html
 *
 * @author doornenbalm
 */
public class TrixHandler {
	...
	public void endElement(EndElement ee) {
		...
		case TRIX + "triple":
			...
			if (rdfItems.get(2).isLiteral()) {
				rdf.assertTriple(rdfItems.get(0).getString(), rdfItems.get(1).getString(), rdfItems.get(2).getLiteral());
			}
			else {
				rdf.assertTriple(rdfItems.get(0).getString(), rdfItems.get(1).getString(), rdfItems.get(2).getString());
			}
			break;
		case TRIX + "uri":
	...
	/**
	 * Do the things that you have to do by the end of the document.
	 * If you assigned DUMMY_LABEL_RESOURCE then ALL of these must be replaced by real resource IDs.
	 */
	void endDocument() {
		if (verbose) {
