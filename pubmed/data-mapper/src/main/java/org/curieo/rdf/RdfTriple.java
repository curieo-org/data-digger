...
public interface RdfTriple {
	...
	/**
	 * Set the URI of this triple.
	 * If this triple is read-only or anonymous this will throw an {@link UnsupportedOperationException}.
	 * @param u
	 */
	...
	default String getSubjectNoAnchor() {
		if (getSubject().indexOf('#') >= 0)
			return getSubject().substring(0, getSubject().indexOf('#'));
		return getSubject();
	}
	...
	/**
	 * Get the object as a literal.
	 * This will not be implemented for all triple types.
	...
	default String asRdfXml(NamespaceService ns) {
		...
		if (this.isLiteral()) {
			...
			String language = l.getLanguage() == null ? "" : 
				String.format(" xml:lang=\"%s\"", l.getLanguage());
			if (l.getValue() == null) {
				LOGGER.error("Value cannot be null in a Literal");
			}
			return "<" + verb + datatype + language + ">" + 
