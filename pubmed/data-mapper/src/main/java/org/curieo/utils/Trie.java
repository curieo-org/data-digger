...
public class Trie<T> {
	...
	/**
	 * Assign values to all intermediate states that lead to a unique value.
	 * In other words, suppose you have these strings:
	 * - marius
	 * - mario
	 * - luigi
	 * - lorenzo
	 * all nodes from the 'u' and 'o' in (marius/mario) get the value that is associated
	 * with 'marius' and 'mario' respectively, same for all nodes after 'uigi' and 'orenzo'.
	 * The allows you to parse partial strings (prefixes).
	 */
