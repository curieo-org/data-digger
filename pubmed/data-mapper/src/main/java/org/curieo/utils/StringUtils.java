...
public class StringUtils {
	 ...
	 * @param s
	 * @return a list of strings. The first element is the original string, with all email addresses removed. 
	 * The other elements are the removed email addresses.
	 */
	...
	private static int doubleDot(Range toRemove, String s) {
		...
		if (start > 0 && (s.charAt(start-1) == '.' || s.charAt(start-1) == ';' || s.charAt(start-1) == ',')) {
			start--;
		}
		return start;
	}
	...
	private static boolean bracketed(Range range, String s) {
		return range.getStart() > 0 && range.getEnd() < s.length() &&
				s.charAt(range.getEnd()) == ')' && s.charAt(range.getStart()-1) =='(';
	}
	...
	private static int name(String text, int offset) {
		while (offset != text.length() &&
				((text.charAt(offset) == '.') || ((text.charAt(offset) < (char)128) && NAME_CHARACTERS[text.charAt(offset)]))) {
			offset++;
		}
		return offset;
	}
