...
public class HashSet<E> extends AbstractSet<E> implements Serializable {
  ...
  private static final long serialVersionUID = -5738076001895762640L;
  ...
  /**
   * In the interest of memory-savings, we start with the smallest feasible power-of-two table size
   * that can hold three items without rehashing. If we started with a size of 2, we'd have to
   * expand as soon as the second item was added.
   */
  private static final int INITIAL_TABLE_SIZE = 4;

  private static final Object NULL_ITEM =
      new Serializable() {
        private static final long serialVersionUID = -3014902729785063728L;
  ...
  /**
   * Number of objects in this set; transient due to custom serialization. Default access to avoid
   * synthetic accessors from inner classes.
   */
  transient int size = 0;

  /**
   * Backing store for all the objects; transient due to custom serialization. Default access to
  ...
  /**
   * Works just like {@link #addAll(Collection)}, but for arrays. Used to avoid having to synthesize
   * a collection in {@link Sets}.
   */
  ...
  /**
   * Removes the item at the specified index, and performs internal management to make sure we don't
   * wind up with a hole in the table. Default access to avoid synthetic accessors from inner
   * classes.
   */
  ...
  /**
   * Returns the index in the table at which a particular item resides, or -1 if the item is not in
   * the table.
   */
  ...
  /**
   * Returns the index in the table at which a particular item resides, or the index of an empty
   * slot in the table where this item should be inserted if it is not already in the table.
   */
  ...
  /**
   * Tricky, we left a hole in the map, which we have to fill. The only way to do this is to search
   * forwards through the map shuffling back values that match this index until we hit a null.
   */
  private void plugHole(int hole) {
    ...
    while (table[index] != null) {
      ...
      if (hole < index) {
        /*
         * "Normal" case, the index is past the hole and the "bad range" is from hole
         * (exclusive) to index (inclusive).
         */
