...
class TrieTest {
  ...
  void testStorageSizeAndSpeed(
      String key, Consumer<String> store, Function<String, Boolean> retrieve) {
    ...
    System.out.printf(
        "%s took %.03f seconds to store 10M integers%n", key, (end - start) / (float) 1000);
    int contained = 0;
    ...
    System.out.printf(
        "%s took %.03f seconds to check 1M integers (%d found)%n",
        key, (last - end) / (float) 1000, contained);
