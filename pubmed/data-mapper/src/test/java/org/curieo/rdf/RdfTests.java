...
class RdfTests {
	...
	private void printStats(Store rdf) {
		...
		rdf.getVerbs().forEach(v-> System.out.printf("verb: %s, with %d subjects and %d objects%n", 
	...
	void testJournals() throws IOException {
		...
		Store rdf = new Turtle(SkosNamespaceService::createStore).read(new File(path)).get(Turtle.DEFAULT);
		printStats(rdf);
	}
