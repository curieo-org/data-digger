...
public class Credentials {
	...
	public String need(String application, String key) {
		...
		if (config == null) {
			throw new IllegalArgumentException(String.format("entry %s not found in config", application));
		}
		
		String retval = config.get(key);
		if (retval == null) {
			throw new IllegalArgumentException(String.format("Key %s not found in application %s in config", key, application));
		}
		return retval;
	}
