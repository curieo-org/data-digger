...
public class DataLoaderPMC {
	...
	static Option queryOption() {
		return Option.builder().option("q").longOpt("query").hasArgs()
				.desc("query for retrieving the PMCs that you want to download").build();
	}
	...
	static Option tableNameOption() {
		return Option.builder().option("t").longOpt("table-name").hasArg().desc("table name for storing full text").build();
	}
	
	public static void main(String[] args) throws ParseException, JsonProcessingException, IOException, SQLException, XMLStreamException {
		...
		Set<String> todo = PostgreSQLClient
				.retrieveSetOfStrings(postgreSQLClient.getConnection(), query);
		LOGGER.info(query);
